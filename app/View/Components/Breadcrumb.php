...
class Breadcrumb
{
    public static function fit_breadcrumb ( $args = array() ): string
    {
        ...
        if ( !is_home() && !is_front_page() && !is_admin() ) {
            $str         .= '<div class="' . $defaults[ 'class' ] . '" >';
            $str         .= '<div class="container" >';
            $str         .= '<ul class="breadcrumb__list">';
            $my_taxonomy = get_query_var( 'taxonomy' );
            $cpt         = get_query_var( 'post_type' );

            $str .= '<li class="breadcrumb__item" itemscope itemtype="http://data-vocabulary.org/Breadcrumb"><a href="' . \home_url( '/' ) . '" itemprop="url"><span itemprop="title">' . $defaults[ 'home' ] . '</span></a></li>';
            ...
            if ( $my_taxonomy && is_tax( $my_taxonomy ) ) {
                ...
                if ( $my_tax->parent != 0 ) {
                    ...
                    foreach ( $ancestors as $ancestor ) {
                        $str .= '<li class="breadcrumb__item" itemscope itemtype="http://data-vocabulary.org/Breadcrumb"><a href="' . \get_term_link( $ancestor,
                                $my_tax->taxonomy ) . '" itemprop="url"><span itemprop="title">' . get_term( $ancestor,
                                $my_tax->taxonomy )->name . '</span></a></li>';
                    }
                }
                ...
                if ( isset( $tax->parent ) ) {
                    if ( $tax->parent != 0 ) {
                        ...
                        foreach ( $ancestors as $ancestor ) {
                            $str .= '<li class="breadcrumb__item" itemscope itemtype="http://data-vocabulary.org/Breadcrumb"><a href="' . \get_term_link( $ancestor,
                                    $mytax ) . '" itemprop="url"><span itemprop="title">' . get_term( $ancestor,
                ...
                if ( get_query_var( 'day' ) != 0 ) {
                    ...
                    $str .= '<li class="breadcrumb__item" itemscope itemtype="http://data-vocabulary.org/Breadcrumb"><a href="' . \get_month_link( get_query_var( 'year' ),
                            get_query_var( 'monthnum' ) ) . '" itemprop="url"><span itemprop="title">' . get_query_var( 'monthnum' ) . '月</span></a></li>';
                ...
                $str .= '<li class="breadcrumb__item">' . $author . get_the_author_meta( 'display_name',
